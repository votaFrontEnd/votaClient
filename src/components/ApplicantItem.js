...
export default class ApplicantItem extends Component {
  render() {
    ...
    const goalsArr = job.parameters.filter(p => {return p.type == 'goal'});
    const skillsArr = job.parameters.filter(p => {return p.type == 'skill'});
    const overallArr = job.parameters.filter(p => {return p.type == 'overall'})
    const ratingsForApplicantArr = job.ratings != null ? job.ratings.filter(rating => {return applicant != null && rating.applicant_id === applicant.Id}) : [];


    
    const myRating = ratingsForApplicantArr.filter(rating => {return rating.interviewer_id === auth.getUser()});
    ...
    for(var i = 0; i < goalsArr.length; i++)
    {
      ...
      {
        const ratingApplicantLength = (ratingsForApplicantArr[j] != null && ratingsForApplicantArr[j].values != null ) ? ratingsForApplicantArr[j].values.length : 0;
      ...
      averagesPerGoal.push({goal: goal, average: sum/ratingsForApplicantArr.length})
    }
    ...
    for(var i = 0; i < skillsArr.length; i++)
    {
      ...
      averagesPerSkill.push({skill: skill, average: sum/ratingsForApplicantArr.length})
    }
      ...
      for(var i = 0; i < ratingValueLength; i++)
      {
        ...
        if (matchingParameter.length > 0)
        {
          ...
          if (parameter.type === 'goal')
          {
            ...
            individualGoalsArr.push({description: parameter.description, value: ratingValue.value})
          }
          else if (parameter.type == 'skill')
          {
            skills += (parameter.weight / 100) * ratingValue.value;
            individualSkillsArr.push({description: parameter.description, value: ratingValue.value})
          }
    ...
    const averageScore = scorePerInterviewer.length > 0 ? scorePerInterviewer.reduce((a, b) => a + b.score, 0)/scorePerInterviewer.length : 'N/A';
    const scorePerGoalDisplay = averagesPerGoal.length > 0 ? averagesPerGoal.map((goal) => <div> {goal.goal.description} - {goal.average} </div>) : 'N/A';
    const scorePerSkillDisplay = averagesPerSkill.length > 0 ? averagesPerSkill.map((skill) => <div> {skill.skill.description} - {skill.average} </div>): 'N/A';
        

    const averagesForGoals = averagesPerGoal.length > 0 ? averagesPerGoal.reduce((a, b) => a + b.average, 0)/averagesPerGoal.length : 'N/A';
    const averagesForSkills = averagesPerSkill.length > 0 ? averagesPerSkill.reduce((a, b) => a + b.average, 0)/averagesPerSkill.length : 'N/A';
    ...
    return (
        <div>
          ...
          <ExpansionPanel>
            <ExpansionPanelSummary expandIcon={<ExpandMoreIcon />}>
              <ScoreLabel label='Reviewers' score={ratingsForApplicantArr.length} hideColor={true} />
            </ExpansionPanelSummary>
            <ExpansionPanelDetails>
              <Typography>
                {scorePerInterviewer.map((interviewer, i) => <InterviewerScores interviewer={interviewer}/>)}
              </Typography>
            </ExpansionPanelDetails>            
          </ExpansionPanel> 
          <Link to={"/addRatings/" + job.id + "/" + applicantID} >
            <Button style={styles.buttons} variant="contained" color="primary" disabled={disableRatingButton} onClick={this.handleAddApplicantsClickOpen}>Rate!</Button>
          </Link>
        </Paper>
        </div>
    )
