...
class AddRating extends Component {
    ...
    if (parameterForGoal.length != 0)
    {
      this.setState({
        parameters: this.state.parameters.map(parameter => {
          return parameter.parameter_id == goal.id ? { ...parameter, value: value }: parameter;
        })
      })
    }
    else{
  ...
  handleSkillRatingChange = (value, skill) => {
    ...
    if (parameterForSkill.length != 0)
    {
      this.setState({
        parameters: this.state.parameters.map(parameter => {
          return parameter.parameter_id == skill.id ? { ...parameter, value: value }: parameter;
        })
      })
    }
    else{
  ...
  handleCommentsChange =  (e, skill) => {
    ...
    if (parameterForSkill.length != 0)
    {
      this.setState({
        parameters: this.state.parameters.map(parameter => {
          return parameter.parameter_id == skill.id ? { ...parameter, description: value }: parameter;
        })
      })
    }
    else{
  ...
  getGoals(job){
    if (job != null && job.parameters != null)
    {
      ...
      var goalsWithRatingArr = goalsArr.map(goal => {
        ...
        newGoal.rating = matchingParameters.length > 0 ? matchingParameters[0].value : 0
        return newGoal;
      })
  ...
  getSkills(job){
    if (job != null && job.parameters != null)
    {
      ...
      var skillsWithRatingArr = skillsArr.map(skill => {
        ...
        newSkill.rating = matchingParameters.length > 0 ? matchingParameters[0].value : 0
        return newSkill;
      })
